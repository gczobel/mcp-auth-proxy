...
- There is deliberately **no** `REQUIRE_EXPLICIT_AUTHORIZATION` flag: the consent screen stays on by default for every deployment. Auto-approval is permitted by OAuth 2.1 §4.1.1 / RFC 6749 §4.1.1, but Anthropic's connector docs describe a standard consent screen, and for a home deployment exposed to the internet the visible "who is requesting access" step (client name + redirect-URI host) is the security value of this proxy. Claude.ai compatibility comes from the replay window, not from disabling the step — so a flag that turns it off would trade away that value for no observed benefit. If a client ever genuinely cannot render the consent step, revisit with a flag then.
